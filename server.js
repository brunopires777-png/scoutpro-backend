...
app.get('/api/leagues', async (req, res) => {
  try {
    ...
    // Busca ID real da Copa do Mundo 2026
    const results = data.results || [];
    const wc2026 = results.find(l =>
      l.name?.toLowerCase().includes('world cup') ||
      l.name?.toLowerCase().includes('fifa world cup') ||
      l.name?.toLowerCase().includes('copa do mundo') ||
      l.name?.toLowerCase().includes('mundial')
    );
    if (wc2026) {
      console.log('[leagues] Copa do Mundo encontrada:', wc2026.name, 'id=', wc2026.id);
    } else {
      // Garante que Copa do Mundo aparece mesmo sem estar na lista de ligas da BSD
      // Busca separada pelo nome
      try {
        const wcsearch = await fetch('https://sports.bzzoiro.com/api/leagues/?search=World+Cup&limit=5', {
          headers: { Authorization: `Token ${BSD_TOKEN}` }
        }).then(r => r.json());
        const wc = (wcsearch.results||[]).find(l => l.name?.toLowerCase().includes('world cup'));
        if (wc) {
          console.log('[leagues] Copa do Mundo via search:', wc.name, 'id=', wc.id);
          data.results = [...results, wc];
        }
      } catch(_) {}
    }
...
// Ligas bloqueadas — filtradas em todos os endpoints de jogos
const LIGAS_BLOQUEADAS = [
  'veikkaus','allsvenskan','caf champions','concacaf gold cup',
  'afc asian cup 2023','africa cup of nations 2023',
  'copa america 2024','copa américa 2024','mls'
];
...
app.get('/api/jogos/hoje', async (req, res) => {
  try {
    ...
    const data = await fetch(url, { headers: { Authorization: `Token ${BSD_TOKEN}` } }).then(r => r.json());
    data.results = filtrarLigasBloqueadas(await enriquecerArbitros((data.results || []).map(normEvento)));
    res.json(data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/jogos/ao-vivo', async (req, res) => {
  try {
    ...
    const normalized = filtrarLigasBloqueadas(await enriquecerArbitros(lista.map(normEvento)));
...
app.get('/api/jogos/:id', async (req, res) => {
  try {
    ...
    // ai_preview pode vir como string, objeto {text:...}, ou {preview:...}
    const rawAi = metaData?.ai_preview || metaData?.preview || metaData?.analysis || null;
    const aiText = typeof rawAi === 'string' ? rawAi
      : rawAi?.text || rawAi?.content || rawAi?.preview || rawAi?.analysis
      || (typeof rawAi === 'object' ? JSON.stringify(rawAi) : null);
    res.json({
      evento: evData,
      stats: statsData,
...
app.get('/api/h2h', async (req, res) => {
  try {
    ...
    // Tenta primeiro o endpoint direto por evento (mais preciso)
    if (event_id) {
      try {
        const h2hDirect = await bsd(`/events/${event_id}/h2h/`);
        rawJogos = h2hDirect.results || h2hDirect.h2h || h2hDirect.events || [];
      } catch(_) {}
    }

    // Fallback: busca manual por times (últimos 5 anos)
    if (!rawJogos.length) {
      const dateFrom = new Date(Date.now() - 1825 * 86400000).toISOString().slice(0,10);
      ...
      rawJogos = [...(d1.results||[]), ...(d2.results||[])];
    }

    // Filtra apenas jogos com placar e entre os dois times certos
    const hid = String(home_id), aid = String(away_id);
    const jogosValidos = rawJogos
      .filter(j => {
        const ev = normEvento(j);
        if (ev.home_score == null || ev.away_score == null) return false;
        // Garante que é entre os dois times corretos
        const jHid = String(ev.home_team_id || j.home_team?.id || '');
        const jAid = String(ev.away_team_id || j.away_team?.id || '');
        return (jHid === hid && jAid === aid) || (jHid === aid && jAid === hid);
      })
      .sort((a, b) => new Date(b.event_date) - new Date(a.event_date))
      .slice(0, 10);

    const jogos = jogosValidos.map(j => {
      const ev = normEvento(j);
      return {
        id: j.id,
    ...
    jogos.forEach(j => {
      const homeIsT1 = String(j.home_team_id) === hid;
    ...
    res.json({
      jogos,
      stats: { team1_wins: t1w, team2_wins: t2w, draws, avg_goals: jogos.length ? (totalGoals/jogos.length).toFixed(1) : 0 }
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Detalhe do jogador (team_id) → /api/player/:id/detail
app.get('/api/player/:id/detail', async (req, res) => {
  try {
    const data = await bsd(`/players/${req.params.id}/`);
    res.json({
      id: data.id,
      name: data.name,
      team_id: data.team?.id || data.current_team?.id || null,
      team_name: data.team?.name || data.current_team?.name || null,
