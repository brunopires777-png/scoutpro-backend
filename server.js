...
app.get('/api/value-bets', async (req, res) => {
  try {
    const { league_id, min_edge = 8, limit = 50 } = req.query;
    ...
    for(const chunk of chunks){
      const res2 = await Promise.allSettled(chunk.map(async ev => {
        try{
          ...
          // Exige probabilidade mínima de 33% para ser considerado value bet
          // (elimina apostas em times com chances reais muito baixas)
          const probDn = probD ?? (100 - probH - probA);
          const oh=parseFloat(ev.odds_home||0), ox=parseFloat(ev.odds_draw||0), oa=parseFloat(ev.odds_away||0);
          if(!oh||!oa) return null;

          const bets=[];

          // Casa: só considera se prob >= 33% e odd não é absurda (>8 = muita incerteza)
          if(probH >= 33 && oh <= 8){
            const eH=((probH/100)*oh-1)*100;
            if(eH >= minEdgePct) bets.push({selection:'home',label:`${ev.home_team} vence`,odd:oh,prob:+probH.toFixed(1),edge:+eH.toFixed(1)});
          }
          // Empate: só se prob >= 20%
          if(probDn >= 20 && ox && ox <= 6){
            const eD=((probDn/100)*ox-1)*100;
            if(eD >= minEdgePct) bets.push({selection:'draw',label:'Empate',odd:ox,prob:+probDn.toFixed(1),edge:+eD.toFixed(1)});
          }
          // Fora: só considera se prob >= 33% e odd não é absurda
          if(probA >= 33 && oa <= 8){
            const eA=((probA/100)*oa-1)*100;
            if(eA >= minEdgePct) bets.push({selection:'away',label:`${ev.away_team} vence`,odd:oa,prob:+probA.toFixed(1),edge:+eA.toFixed(1)});
          }

          if(!bets.length) return null;
          // Retorna as três probabilidades para os gráficos de donut
          return { evento:ev, bets, probs:{ home:+probH.toFixed(1), draw:+probDn.toFixed(1), away:+probA.toFixed(1) } };
        }catch(_){return null;}
      }));
