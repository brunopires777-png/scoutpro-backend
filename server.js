...
const app = express();
app.set('trust proxy', 1); // Render fica atrás de um proxy — necessário pra req.protocol/host funcionar certo com o domínio custom
...
// ─────────────────────────────────────────────
// PWA — manifest, service worker e ícones (permite "Instalar app")
// ─────────────────────────────────────────────
...
app.get('/stadium.jpg', (req, res) => {
  const p = path.join(__dirname, 'stadium.jpg');
  if (require('fs').existsSync(p)) res.sendFile(p);
  else res.status(404).end();
});
// Screenshots reais usados na página /apresentacao (visão mobile e desktop do app)
['screenshot-mobile.jpg','screenshot-desktop.jpg'].forEach(f => {
  app.get('/'+f, (req, res) => {
...
app.get('/apresentacao', (req, res) => {
  res.sendFile(path.join(__dirname, 'apresentacao-esa.html'));
});
app.use('/icons', express.static(path.join(__dirname, 'icons')));
// Ícones também na raiz do projeto (é onde os arquivos foram enviados)
...
// ─────────────────────────────────────────────
// PRESENÇA — contador de usuários online (em memória, sem Firebase)
// ─────────────────────────────────────────────
const PRESENCE_WINDOW_MS = 70000; // considera "online" quem fez ping nos últimos 70s
const presenceMap = new Map(); // sessionId -> timestamp do último ping
function presenceCount() {
  const limite = Date.now() - PRESENCE_WINDOW_MS;
  let n = 0;
  for (const ts of presenceMap.values()) if (ts >= limite) n++;
  return n;
}
// Limpa sessões antigas periodicamente pra não crescer sem limite
setInterval(() => {
  const limite = Date.now() - PRESENCE_WINDOW_MS;
  for (const [id, ts] of presenceMap.entries()) if (ts < limite) presenceMap.delete(id);
}, 5 * 60000);

app.post('/api/presence/ping', (req, res) => {
  const sid = (req.body && req.body.sid) || req.query.sid;
  if (sid) presenceMap.set(String(sid), Date.now());
...
// ─────────────────────────────────────────────
// LETREIRO — melhores jogadores do dia
// Busca jogos ao vivo/encerrados das ligas selecionadas e retorna
// os jogadores com maior rating. Cache de 30min para não sobrecarregar.
// ─────────────────────────────────────────────
const LETREIRO_LIGAS = [9, 34, 7, 1, 5, 52, 27, 3, 4]; // Brasileirão A/B, Champions, Premier, Bundesliga, Chinese Super League, Copa do Mundo 2026, La Liga, Serie A (Itália)
// IDs confirmados na BSD (sports.bzzoiro.com/leagues/):
// 9=Brasileirão A · 34=Brasileirão B · 7=Champions League · 1=Premier League ·
// 5=Bundesliga · 52=Chinese Super League · 27=Copa do Mundo 2026 · 3=La Liga · 4=Serie A (Itália)
let _letreiroCache = null;
let _letreiroCacheTs = 0;
const LETREIRO_TTL = 30 * 60 * 1000; // 30 min

app.get('/api/letreiro', async (req, res) => {
  try {
    if (_letreiroCache && Date.now() - _letreiroCacheTs < LETREIRO_TTL) {
      return res.json(_letreiroCache);
    }
    const t = today();
    ...
    // Busca jogos das ligas alvo que estão ao vivo ou encerrados hoje.
    // Usa /api/v2/events/ (o /api/events/ v1 é depreciado e pode não ter ligas novas
    // como a Copa do Mundo 2026 sincronizadas). Param correto na v2 é "league_id".
    const jogosPromises = LETREIRO_LIGAS.map(lgId =>
      bsd('/events/', { date_from: tPrev, date_to: tNext, league_id: lgId, limit: 20 }).catch(() => [])
    );
    const resultados = await Promise.all(jogosPromises);
    const VIVOS_FIN = new Set(['inprogress','1st_half','2nd_half','halftime','extratime','extra_time','penalties','finished','aet','ft','complete']);
    const jogosAtivos = resultados.flatMap(d => {
      const arr = Array.isArray(d) ? d : (d?.results || []); // v2 retorna array puro, sem wrapper .results
      return arr.filter(ev => VIVOS_FIN.has((ev.status||'').toLowerCase()));
    });

    if (!jogosAtivos.length) {
      _letreiroCache = { players: [] };
      _letreiroCacheTs = Date.now();
      return res.json(_letreiroCache);
    }

    // Busca stats dos jogadores em paralelo (máx 25 jogos) via /api/v2/events/{id}/player-stats/
    // Busca também as escalações (/lineups/) — o player-stats às vezes só traz o ID do
    // jogador sem nome, e a escalação é a fonte confiável de nome + time (mesmo padrão
    // já usado no modal de jogo, aba Jogadores).
    const slice = jogosAtivos.slice(0, 25);
    const statsPromises = slice.map(ev =>
      Promise.all([
        bsd(`/events/${ev.id}/player-stats/`).catch(() => null),
        bsd(`/events/${ev.id}/lineups/`).catch(() => null)
      ]).then(([stats, lineups]) => ({ ev, stats, lineups }))
    );
    const statsResultados = await Promise.all(statsPromises);

    // Extrai jogadores com rating, filtra os sem dados.
    // Campos confirmados na v2 (mesmo formato usado no modal de jogo, aba Jogadores):
    // rating vem plano em p.rating; id do jogador em p.player_id/p.player.id/p.id.
    const jogadores = [];
    for (const sr of statsResultados) {
      if (!sr) continue;
      const { ev, stats, lineups } = sr;
      const todos = stats?.player_stats || stats?.results || (Array.isArray(stats) ? stats : []);

      // Mapa id do jogador → {nome, teamId} a partir da escalação
      const luMap = new Map();
      const lu = lineups?.lineups;
      if (lu) {
        ['home','away'].forEach(side => {
          const s = lu[side];
          if (!s) return;
          const sideTeamId = side === 'home' ? ev.home_team_id : ev.away_team_id;
          [...(s.players||[]), ...(s.substitutes||[])].forEach(pl => {
            const pid = String(pl.player_id || pl.id || '');
            if (pid) luMap.set(pid, { name: pl.short_name || pl.name || null, teamId: sideTeamId });
          });
        });
      }

      for (const p of todos) {
        const rating = parseFloat(p.rating ?? p.overall_rating ?? 0);
        if (!rating || rating < 7.5) continue;
        const playerId = p.player_id || p.player?.id || p.id;
        const luInfo = playerId != null ? luMap.get(String(playerId)) : null;
        const isHome = p.is_home || p.side === 'home' || (ev.home_team_id != null && p.team_id === ev.home_team_id);
        const name = p.player?.short_name || p.player?.name || p.player?.display_name || p.player?.full_name
          || p.player_name || p.short_name || p.name || luInfo?.name || '—';
        const teamId = p.team_id || luInfo?.teamId || (isHome ? ev.home_team_id : ev.away_team_id) || null;
        jogadores.push({
          id: playerId,
          name,
          photo: playerId ? `${process.env.RENDER_EXTERNAL_URL||'http://localhost:10000'}/img/player/${playerId}` : null,
          teamName: p.team_name || (isHome ? ev.home_team : ev.away_team) || '—',
          teamId,
          position: p.player?.position || p.position || p.pos || '—',
          rating: rating,
          goals: p.goals || 0,
          assists: p.assists || 0,
          league: ev.league?.name || ev.league_name || '—',
          matchLabel: `${ev.home_team||'?'} ${ev.home_score??''}–${ev.away_score??''} ${ev.away_team||'?'}`,
        });
      }
    }
    ...
    const top = jogadores.slice(0, 30);

    _letreiroCache = { players: top };
    _letreiroCacheTs = Date.now();
    res.json(_letreiroCache);
  } catch(e) {
    console.error('[letreiro]', e.message);
    res.json({ players: [] });
  }
});

// ─────────────────────────────────────────────
// GALERA — limpeza automática de palpites expirados (roda a cada hora)
// ─────────────────────────────────────────────
async function limparPalpitesExpirados(){
  try{
    const admin = await import('firebase-admin').catch(()=>null);
    if(!admin) return; // firebase-admin não instalado, pula
    if(!admin.apps.length) return;
    const db = admin.firestore();
    const agora = admin.firestore.Timestamp.now();
    const snap = await db.collection('palpites').where('expiraEm','<=',agora).get();
    const batch = db.batch();
    snap.docs.forEach(d => batch.delete(d.ref));
    if(snap.size > 0){
      await batch.commit();
      console.log(`[galera] ${snap.size} palpites expirados removidos`);
    }
  }catch(e){
    console.log('[galera] limpeza:', e.message);
  }
}
setInterval(limparPalpitesExpirados, 60*60*1000); // a cada hora
limparPalpitesExpirados(); // roda uma vez na inicialização
// ─────────────────────────────────────────────
...
function normEvento(ev) {
  ...
  return {
    ...
    // Placar de prorrogação (só os gols da prorrogação, não acumulado)
    et_home: ev.extra_time_score?.home ?? null,
    et_away: ev.extra_time_score?.away ?? null,
    // Placar de pênaltis
    pen_home: ev.penalty_shootout?.home ?? null,
    pen_away: ev.penalty_shootout?.away ?? null,
  };
}
...
app.get('/api/value-bets', async (req, res) => {
  try {
    ...
    for(const pred of preds){
      ...
      // Over/Under: avalia os três limiares que a BSD calcula (1.5, 2.5, 3.5) e
      // escolhe o de MAIOR confiança — mesma lógica do "Top Pick" mostrado no
      // site da BSD, em vez de checar sempre só o de 2.5 gols.
      const ouCandidatos = [
        { linha:'1.5', raw: ou.prob_over_15, oddsKeys:{over:['over_15','over_1.5'], under:['under_15','under_1.5']} },
        { linha:'2.5', raw: ou.prob_over_25, oddsKeys:{over:['over_25','over_2.5'], under:['under_25','under_2.5']} },
        { linha:'3.5', raw: ou.prob_over_35, oddsKeys:{over:['over_35','over_3.5'], under:['under_35','under_3.5']} }
      ].filter(c => c.raw != null);

      let melhorOU = null;
      for(const c of ouCandidatos){
        const prob = toP(c.raw);
        const confianca = Math.max(prob, 100 - prob);
        if(!melhorOU || confianca > melhorOU.confianca) melhorOU = { linha:c.linha, prob, confianca, oddsKeys:c.oddsKeys };
      }
      if(melhorOU && melhorOU.confianca >= 55){
        const ehOver = melhorOU.prob >= 50;
        const probFinal = ehOver ? melhorOU.prob : (100 - melhorOU.prob);
        const keys = ehOver ? melhorOU.oddsKeys.over : melhorOU.oddsKeys.under;
        const oddOU = parseFloat(keys.map(k => bo[k]?.odds).find(v => v) || 0);
        const edge = oddOU > 1 ? +((probFinal/100*oddOU - 1)*100).toFixed(1) : null;
        bets.push({ tipo:'over_under', selection:(ehOver?'over_':'under_')+melhorOU.linha.replace('.',''),
          label:`${ehOver?'Over':'Under'} ${melhorOU.linha} Gols`,
          odd: oddOU > 1 ? +oddOU.toFixed(2) : null, prob:+probFinal.toFixed(1), edge, icon:'⚽' });
      }
    ...
    const sliced = valueBets.slice(0, parseInt(limit));
    ...
    await Promise.all(sliced.map(async vb => {
      try {
    ...
    // Placar exibido no card só aparece quando vem do texto do Preview (score_ia) —
    // ver frontend. Aqui não filtramos mais os jogos por isso, só enriquecemos.
    console.log(`[value-bets v2] ${preds.length} preds → ${valueBets.length} bets | enviados: ${sliced.length} | com placar IA: ${sliced.filter(v=>v.score_ia).length}`);
...
app.get('/api/jogos/:id/compare-odds', async (req, res) => {
  try {
    ...
    // Formato real da BSD: data.markets['1x2']['HOME']['bookmakers'][slug] = {decimal_odds, movement}
    const m1x2 = data?.markets?.['1x2'];
    if(!m1x2) return res.json({ bookmakers: [] });
    const homeBooks = m1x2.HOME?.bookmakers || {};
    const drawBooks = m1x2.DRAW?.bookmakers || {};
    const awayBooks = m1x2.AWAY?.bookmakers || {};
    // Junta todos os slugs únicos (exceto consensus)
    const slugs = [...new Set([
      ...Object.keys(homeBooks),
      ...Object.keys(drawBooks),
      ...Object.keys(awayBooks)
    ])].filter(s => !s.includes('consensus'));
    const bkms = slugs.map(slug => ({
      bookmaker: slug.replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase()),
      home_win:  homeBooks[slug]?.decimal_odds || null,
      draw:      drawBooks[slug]?.decimal_odds || null,
      away_win:  awayBooks[slug]?.decimal_odds || null,
      home_mov:  homeBooks[slug]?.movement || '',
      draw_mov:  drawBooks[slug]?.movement || '',
      away_mov:  awayBooks[slug]?.movement || '',
    })).filter(b => b.home_win || b.draw || b.away_win);
    // Consensus como primeira linha
    const consensus = {
      bookmaker: 'Consensus (avg)',
      home_win:  m1x2.HOME?.best_odds || null,
      draw:      m1x2.DRAW?.best_odds || null,
      away_win:  m1x2.AWAY?.best_odds || null,
      is_consensus: true,
      best_home_book: m1x2.HOME?.best_bookmaker_name || '',
      best_draw_book: m1x2.DRAW?.best_bookmaker_name || '',
      best_away_book: m1x2.AWAY?.best_bookmaker_name || '',
    };
    res.json({ bookmakers: [consensus, ...bkms], best_home: m1x2.HOME?.best_odds, best_draw: m1x2.DRAW?.best_odds, best_away: m1x2.AWAY?.best_odds });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
const PLANOS_MP = {
  semanal: { titulo: 'ESA Semanal',  valor: 5,   dias: 7   },
...
app.post('/api/criar-pagamento', async (req, res) => {
  try {
    ...
    // Usa o domínio de onde a pessoa realmente está (domínio novo, www, ou o
    // onrender.com de fallback) em vez de fixar sempre no domínio antigo do Render —
    // assim o retorno do pagamento cai no mesmo domínio que o usuário está usando.
    const baseUrl = req.headers.origin || `${req.protocol}://${req.get('host')}` || process.env.RENDER_EXTERNAL_URL || `http://localhost:${PORT}`;
    ...
    // init_point = checkout de produção real | sandbox_init_point = ambiente de teste
    // (antes estava priorizando o sandbox por engano, mesmo com token de produção)
...
<ul>
  <li><strong>Semanal:</strong> R$ 5,00 por semana (7 dias)</li>
