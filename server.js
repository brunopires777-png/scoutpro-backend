...
app.get('/api/value-bets', async (req, res) => {
  try {
    ...
    const preds = Array.isArray(predRaw) ? predRaw : (predRaw?.results || []);
    console.log(`[value-bets] predRaw type=${Array.isArray(predRaw)?'array':'object'} len=${preds.length} keys=${!Array.isArray(predRaw)?Object.keys(predRaw||{}).join(','):'-'}`);
