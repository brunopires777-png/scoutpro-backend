...
app.get('/api/value-bets', async (req, res) => {
  try {
    ...
    const minEdgePct = parseFloat(min_edge); // já em %

    // Busca jogos dos próximos 5 dias (Copa do Mundo: fase de grupos encerra, knockout começa)
    const t     = today();
    const tNext = dayOffset(5);
    const params = { date_from: t, date_to: tNext, limit: 200, tz: 'America/Sao_Paulo' };
    ...
    console.log(`[value-bets] ${todos.length} eventos (${t} a ${tNext}) → ${jogos.length} futuros`);
    if(!jogos.length){
      return res.json({
        results:[], total:0, source:'internal_calc',
        _diag: { eventos_total: todos.length, janela: `${t} a ${tNext}`, status_vistos: [...new Set(todos.map(ev=>ev.status||ev.event_status||'?'))] }
      });
    }

    // Busca odds para os eventos que vieram sem (igual enriquecerOdds)
    const semOdds = jogos.filter(ev=>!ev.odds_home&&!ev.odds_draw&&!ev.odds_away&&ev.id);
    if(semOdds.length){
      const oddsChunks=[];
      for(let i=0;i<semOdds.length;i+=20) oddsChunks.push(semOdds.slice(i,i+20));
      const oddsMap=new Map();
      for(const chunk of oddsChunks){
        await Promise.allSettled(chunk.map(async ev=>{
          try{
            const d=await bsd(`/api/v2/events/${ev.id}/odds/`);
            const m=d?.markets?.['1X2']||d?.markets?.['1x2']||d?.['1X2']||{};
            const oh=parseFloat(m.HOME||m.home||0)||null;
            const ox=parseFloat(m.DRAW||m.draw||m.X||0)||null;
            const oa=parseFloat(m.AWAY||m.away||0)||null;
            if(oh&&oa) oddsMap.set(ev.id,{odds_home:oh,odds_draw:ox,odds_away:oa});
          }catch(_){}
        }));
      }
      jogos=jogos.map(ev=>oddsMap.has(ev.id)?{...ev,...oddsMap.get(ev.id)}:ev);
    }

    // Para cada jogo, busca predição e calcula edge
    const valueBets = [];
    const CONCURRENT = 6;
    const chunks = [];
    for(let i=0;i<jogos.length;i+=CONCURRENT) chunks.push(jogos.slice(i,i+CONCURRENT));

    for(const chunk of chunks){
      const results = await Promise.allSettled(chunk.map(async ev => {
        try{
          const pred = await bsd(`/events/${ev.id}/prediction/`);
          const markets = pred?.markets || {};
          const mRes = markets.match_result || {};
          let probH = mRes.prob_home, probD = mRes.prob_draw, probA = mRes.prob_away;
          if(probH==null || probA==null) return null;

          // BSD retorna tanto 0.60 (decimal) quanto 60.0 (percentual) — detecta automaticamente
          const isDecimal = probH <= 1 && probA <= 1;
          if(isDecimal){ probH*=100; if(probD!=null)probD*=100; probA*=100; }
          if(!probH || !probA || probH<1 || probA<1) return null;

          const norm = normEvento(ev);
          const oh = parseFloat(norm.odds_home||ev.odds_home||0);
          const ox = parseFloat(norm.odds_draw||ev.odds_draw||0);
          const oa = parseFloat(norm.odds_away||ev.odds_away||0);
          if(!oh || !oa) return null;

          // edge% = (prob_ia/100 × odd - 1) × 100
          const bets = [];
          const edgeH = ((probH/100)*oh - 1)*100;
          const edgeD = probD!=null ? ((probD/100)*ox - 1)*100 : null;
          const edgeA = ((probA/100)*oa - 1)*100;

          if(edgeH >= minEdgePct) bets.push({ selection:'home', label:`${norm.home_team} vence`, odd:oh, prob:+probH.toFixed(1), edge:+edgeH.toFixed(1) });
          if(edgeD!=null && ox && edgeD>=minEdgePct) bets.push({ selection:'draw', label:'Empate', odd:ox, prob:+probD.toFixed(1), edge:+edgeD.toFixed(1) });
          if(edgeA >= minEdgePct) bets.push({ selection:'away', label:`${norm.away_team} vence`, odd:oa, prob:+probA.toFixed(1), edge:+edgeA.toFixed(1) });

          if(!bets.length) return null;
          return { evento: norm, bets };
        }catch(_){return null;}
      }));
      results.forEach(r=>{ if(r.status==='fulfilled'&&r.value) valueBets.push(r.value); });
    }
    ...
    console.log(`[value-bets-interno] ${jogos.length} avaliados → ${valueBets.length} com edge ≥ ${min_edge}% → ${sliced.length} retornados`);
