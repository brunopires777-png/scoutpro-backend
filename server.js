...
app.get('/api/jogos/:id', async (req, res) => {
  try {
    ...
    const aiText = (typeof aiObj === 'string' && aiObj.length > 5) ? aiObj
      : (aiObj?.text && aiObj.text.length > 5) ? aiObj.text : null;
    console.log(`[jogo ${id}] ai_preview: ${aiText ? 'OK '+aiText.slice(0,40) : 'NULL'} | meta keys: ${metaData ? Object.keys(metaData).join(',') : 'null'}`);
    res.json({
      evento: evData,
      stats: statsData,
...
app.get('/api/value-bets', async (req, res) => {
  try {
    ...
    // Busca predições com pelo menos uma recomendação ativa (recommended=true)
    const predQs = new URLSearchParams({ status: 'upcoming', recommended: 'true', limit: '200' });
    if(league_id) predQs.set('league_id', league_id);

    // Busca melhores odds disponíveis por evento
    const oddsQs = new URLSearchParams({ limit: '300' });

    const [predRaw, oddsRaw] = await Promise.all([
      fetch(`https://sports.bzzoiro.com/api/v2/predictions/?${predQs}`, {
        headers: { Authorization: `Token ${BSD_TOKEN}` }
      }).then(r => r.json()),
      fetch(`https://sports.bzzoiro.com/api/v2/odds/best/?${oddsQs}`, {
        headers: { Authorization: `Token ${BSD_TOKEN}` }
      }).then(r => r.json()).catch(() => [])
    ]);

    // BSD retorna paginado: {count, next, previous, results:[]}
    let preds = Array.isArray(predRaw) ? predRaw : (predRaw?.results || []);
    console.log(`[value-bets] recommended: ${preds.length} jogos`);

    // Fallback: se não há recomendações, busca com min_confidence
    if(!preds.length){
      const fbQs = new URLSearchParams({ status:'upcoming', min_confidence:'0.45', limit:'150' });
      if(league_id) fbQs.set('league_id', league_id);
      const fbRaw = await fetch(`https://sports.bzzoiro.com/api/v2/predictions/?${fbQs}`,{
        headers: { Authorization: `Token ${BSD_TOKEN}` }
      }).then(r=>r.json()).catch(()=>({}));
      const fbArr = Array.isArray(fbRaw) ? fbRaw : (fbRaw?.results || []);
      // Filtra: modelo tem predição clara e confiança >= 52%
      preds = fbArr.filter(p => p.markets?.match_result?.predicted !== null
        && p.model?.confidence >= 0.52);
      console.log(`[value-bets] fallback confidence: ${preds.length} jogos`);
    }
    ...
    for(const pred of preds){
      ...
      // Probabilidades (0-1) → converter para %
      const probH  = (mr.prob_home  || 0) * 100;
      const probD  = (mr.prob_draw  || 0) * 100;
      const probA  = (mr.prob_away  || 0) * 100;

      // Melhores odds disponíveis
      const bestOdds = oddsMap.get(String(ev.id)) || {};
      const bo = bestOdds.best_odds || bestOdds.outcomes || {};

      // Monta lista de bets recomendados
      const bets = [];

      // 1x2 recomendados
      if(rec.winner || rec.bet_favorite){
        ...
        if(probH >= 30 && oh > 1){
          const edge = ((probH/100)*oh - 1)*100;
          if(edge >= 5) bets.push({ tipo:'1x2', selection:'home', label:`${ev.home_team} Vence`, odd:+oh.toFixed(2), prob:+probH.toFixed(1), edge:+edge.toFixed(1), icon:'🏠' });
        }
        if(probD >= 20 && ox > 1){
          const edge = ((probD/100)*ox - 1)*100;
          if(edge >= 5) bets.push({ tipo:'1x2', selection:'draw', label:'Empate', odd:+ox.toFixed(2), prob:+probD.toFixed(1), edge:+edge.toFixed(1), icon:'🤝' });
        }
        if(probA >= 30 && oa > 1){
          const edge = ((probA/100)*oa - 1)*100;
          if(edge >= 5) bets.push({ tipo:'1x2', selection:'away', label:`${ev.away_team} Vence`, odd:+oa.toFixed(2), prob:+probA.toFixed(1), edge:+edge.toFixed(1), icon:'✈️' });
        }
      }

      // Over 2.5
      if(rec.over_25){
        const probO25 = (ou.prob_over_25 || 0)*100;
        const oddO25  = parseFloat(bo.over_25?.odds || bo['over_2.5']?.odds || 0);
        if(probO25 >= 30){
          const edge = oddO25 > 1 ? ((probO25/100)*oddO25 - 1)*100 : probO25 - 50;
          bets.push({ tipo:'over_under', selection:'over_25', label:'Over 2.5 Gols', odd: oddO25 > 1 ? +oddO25.toFixed(2) : null, prob:+probO25.toFixed(1), edge:+edge.toFixed(1), icon:'⚽' });
        }
      }

      // BTTS
      if(rec.btts){
        const probBtts = (btts.prob_yes || btts.probability || 0)*100;
        const oddBtts  = parseFloat(bo.btts_yes?.odds || bo['btts']?.odds || 0);
        if(probBtts >= 40){
          const edge = oddBtts > 1 ? ((probBtts/100)*oddBtts - 1)*100 : probBtts - 50;
          bets.push({ tipo:'btts', selection:'btts', label:'Ambas Marcam (Sim)', odd: oddBtts > 1 ? +oddBtts.toFixed(2) : null, prob:+probBtts.toFixed(1), edge:+edge.toFixed(1), icon:'🎯' });
        }
      }

      // Só inclui se tiver pelo menos 1 bet
      if(!bets.length && (rec.winner || rec.bet_favorite || rec.over_25 || rec.btts)){
        // Sem odds disponíveis mas o modelo recomenda — inclui sem edge
        const mainProb = mr.predicted === 'H' ? probH : mr.predicted === 'A' ? probA : probD;
        const mainLabel = mr.predicted === 'H' ? `${ev.home_team} Vence` : mr.predicted === 'A' ? `${ev.away_team} Vence` : 'Empate';
        bets.push({ tipo:'1x2', selection: mr.predicted==='H'?'home':mr.predicted==='A'?'away':'draw',
          label: mainLabel, odd: null, prob:+mainProb.toFixed(1), edge: null, icon: mr.predicted==='H'?'🏠':mr.predicted==='A'?'✈️':'🤝' });
      }
      ...
      valueBets.push({
        evento: {
          id: ev.id,
          home_team: ev.home_team || ev.home?.name || '?',
          away_team: ev.away_team || ev.away?.name || '?',
          home_team_id: ev.home?.id,
          away_team_id: ev.away?.id,
          event_date: ev.start_time || ev.datetime,
          league_name: ev.league?.name || ev.competition?.name || '',
          league_id: ev.league?.id || ev.competition?.id
        },
    ...
    console.log(`[value-bets v2] ${preds.length} preds recomendadas → ${valueBets.length} com bets`);
