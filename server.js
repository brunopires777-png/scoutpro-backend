...
app.get('/api/jogos/:id', async (req, res) => {
  try {
    ...
    // Campo confirmado nos logs: ai_preview (pode vir null para alguns jogos na BSD)
    let aiText = null;
    let aiFunFacts = null;
    if (metaData) {
      // ai_preview: texto principal da análise
      const rawAi = metaData.ai_preview;
      if (rawAi && typeof rawAi === 'string' && rawAi.trim().length > 10) {
        aiText = rawAi.trim();
      }
      // funfacts: curiosidades do jogo — usa como alternativa quando ai_preview é null
      const rawFf = metaData.funfacts;
      if (rawFf) {
        if (typeof rawFf === 'string' && rawFf.trim().length > 10) {
          aiFunFacts = rawFf.trim();
        } else if (Array.isArray(rawFf) && rawFf.length) {
          aiFunFacts = rawFf.map((f,i) => `• ${typeof f === 'string' ? f : f.text || f.fact || JSON.stringify(f)}`).join('\n');
        }
      }
      if (!aiText) {
        console.log(`[ai_preview] campo ai_preview vazio/null para evento ${id} | funfacts: ${aiFunFacts ? 'sim' : 'não'}`);
      }
    }
    res.json({
      evento: evData,
