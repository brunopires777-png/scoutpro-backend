...
app.get('/api/value-bets', async (req, res) => {
  try {
    const { league_id, min_edge = 5, limit = 50 } = req.query;
    const minEdge = parseFloat(min_edge) / 100; // converte % para decimal (5% → 0.05)

    // Busca jogos de hoje e amanhã (janela de apostas relevante)
    const t     = today();
    const tNext = (() => { const d = new Date(t); d.setDate(d.getDate()+1); return d.toISOString().slice(0,10); })();
    const params = { date_from: t, date_to: tNext, limit: 200, tz: 'America/Sao_Paulo', full: true };
    if(league_id) params.league_id = league_id;

    const data  = await bsd('/events/', params);
    const jogos = (data.results || []).filter(ev => {
      // Só jogos futuros (ainda não começaram)
    ...
    for(const chunk of chunks){
      const results = await Promise.allSettled(chunk.map(async ev => {
        try{
          ...
          const probH = mRes.prob_home, probD = mRes.prob_draw, probA = mRes.prob_away;
          if(!probH || !probA) return null;

          const norm = normEvento(ev);
          const oh = norm.odds_home, ox = norm.odds_draw, oa = norm.odds_away;
          if(!oh || !oa) return null;

          // edge = (prob_ia / odd) - 1
          // Ex: prob 60% (0.60) e odd 2.0 → edge = 0.60*2.0 - 1 = 0.20 (20%)
          const bets = [];
          const edgeH = (probH/100) * oh - 1;
          const edgeD = probD!=null ? (probD/100) * ox - 1 : null;
          const edgeA = (probA/100) * oa - 1;

          if(edgeH >= minEdge) bets.push({ market:'match_result', selection:'home', label:`${norm.home_team} vence`, odd:oh, prob:probH, edge:+(edgeH*100).toFixed(1) });
          if(edgeD!=null && edgeD >= minEdge) bets.push({ market:'match_result', selection:'draw', label:'Empate', odd:ox, prob:probD, edge:+(edgeD*100).toFixed(1) });
          if(edgeA >= minEdge) bets.push({ market:'match_result', selection:'away', label:`${norm.away_team} vence`, odd:oa, prob:probA, edge:+(edgeA*100).toFixed(1) });

          if(!bets.length) return null;
          return { evento: norm, bets, pred };
        }catch(_){return null;}
      }));
    ...
    console.log(`[value-bets-interno] ${jogos.length} jogos avaliados → ${valueBets.length} com edge ≥ ${min_edge}%`);
    res.json({ results: sliced, total: sliced.length, source: 'internal_calc', min_edge: parseFloat(min_edge) });
  } catch(e) {
