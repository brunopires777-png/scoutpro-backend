...
app.get('/api/jogos/:id', async (req, res) => {
  try {
    ...
    const [evento, stats, incidents, odds, lineups, playerStats, predicao, metadata, eventoBase] = await Promise.allSettled([
      ...
      bsd(`/events/${id}/odds/`).catch(() => bsd(`/events/${id}/odds/comparison/`)),
      ...
      odds: (function() {
        ...
        if (arr && arr.length) {
          ...
          const bookmakers = arr.map(b => ({
            ...
            home_win: parseFloat(b.home_win || b.home || b['1'] || 0) || null,
            draw:     parseFloat(b.draw || b.x || b['X'] || 0) || null,
            away_win: parseFloat(b.away_win || b.away || b['2'] || 0) || null,
        ...
        // Fallback: odds simples — tenta no objeto od direto também
        const ev2 = evData || {};
        const hw = od?.home_win ?? od?.home ?? od?.['1'] ?? ev2.odds_home ?? ev2.win_home ?? null;
        const dr = od?.draw ?? od?.x ?? od?.['X'] ?? ev2.odds_draw ?? ev2.win_draw ?? null;
        const aw = od?.away_win ?? od?.away ?? od?.['2'] ?? ev2.odds_away ?? ev2.win_away ?? null;
        const o15 = od?.over_15_goals ?? od?.over15 ?? null;
        const o25 = od?.over_25_goals ?? od?.over25 ?? ev2.over_25 ?? null;
        const o35 = od?.over_35_goals ?? od?.over35 ?? null;
        const u25 = od?.under_25_goals ?? od?.under25 ?? ev2.under_25 ?? null;
        const btts = od?.btts_yes ?? od?.btts ?? ev2.btts ?? null;
