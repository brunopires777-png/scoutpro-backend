...
app.get('/api/letreiro', async (req, res) => {
  try {
    ...
    const jogosPromises = LETREIRO_LIGAS.map(lgId =>
      fetch(`https://sports.bzzoiro.com/api/events/?date_from=${tPrev}&date_to=${tNext}&league=${lgId}&limit=20`, {
        headers: { Authorization: `Token ${BSD_TOKEN}` }
      }).then(r => r.json()).catch(() => ({ results: [] }))
    );
    const resultados = await Promise.all(jogosPromises);
    const VIVOS_FIN = new Set(['inprogress','1st_half','2nd_half','halftime','extra_time','penalties','finished','aet','ft','complete']);
    const jogosAtivos = resultados.flatMap(d => (d.results||[]).filter(ev => {
      const s = (ev.status||'').toLowerCase();
      return VIVOS_FIN.has(s);
    }));
    ...
    // Busca stats dos jogadores em paralelo (máx 25 jogos)
    const slice = jogosAtivos.slice(0, 25);
    const statsPromises = slice.map(ev =>
      fetch(`https://sports.bzzoiro.com/api/events/${ev.id}/player-stats/`, {
        headers: { Authorization: `Token ${BSD_TOKEN}` }
      }).then(r => r.json()).then(d => ({ ev, stats: d })).catch(() => null)
    );
    ...
    for (const sr of statsResultados) {
      if (!sr) continue;
      const { ev, stats } = sr;
      const lista = stats.results || stats.home_players || [];
      const todos = Array.isArray(lista) ? lista :
        [...(stats.home_players||[]), ...(stats.away_players||[])];
      for (const p of todos) {
        const rating = parseFloat(p.rating || p.stats?.rating || 0);
        if (!rating || rating < 6) continue;
        jogadores.push({
          id: p.player_id || p.id,
          name: p.player_name || p.name || '—',
          photo: p.player_id ? `${process.env.RENDER_EXTERNAL_URL||'http://localhost:10000'}/img/player/${p.player_id}` : null,
          teamName: p.team_name || (p.team === ev.home_team ? ev.home_team : ev.away_team) || '—',
