...
// Debug: ver metadata cru de um evento para diagnóstico de ai_preview ausente
...
app.get('/api/jogos/:id', async (req, res) => {
  try {
    ...
    // Tenta extrair o texto de análise/preview de todos os campos possíveis que a BSD usa
    // (o schema documenta "AI-generated preview text" mas não especifica o nome do campo)
    let aiText = null;
    if (metaData) {
      // Busca recursiva em todos os campos string longos do metadata
      const _tryExtract = (obj, depth = 0) => {
        if (!obj || depth > 3) return null;
        for (const [k, v] of Object.entries(obj)) {
          if (typeof v === 'string' && v.length > 40) {
            // Prioriza campos com nomes relacionados a preview/análise
            if (/preview|analysis|ai|text|description|content|review|scout/i.test(k)) return v;
          }
        }
        // Segunda passagem: qualquer string longa que pareça texto analítico
        for (const [k, v] of Object.entries(obj)) {
          if (typeof v === 'string' && v.length > 100) return v;
          if (typeof v === 'object' && v && depth < 2) {
            const found = _tryExtract(v, depth + 1);
            if (found) return found;
          }
        }
        return null;
      };
      aiText = _tryExtract(metaData);
      if (!aiText) {
        console.log(`[ai_preview] metadata campos disponíveis para evento ${id}:`, Object.keys(metaData).join(', '));
      }
    } else {
      console.log(`[ai_preview] metadata não retornou dados para evento ${id}`);
    }
    res.json({
      evento: evData,
      stats: statsData,
...
app.get('/api/value-bets', async (req, res) => {
  try {
    const { league_id, min_edge = 4, limit = 50 } = req.query;
    const minEdgePct = parseFloat(min_edge);

    // Usa v1 /api/events/ igual às rotas hoje/amanhã — retorna todas as ligas
    const t  = today();
    const t5 = dayOffset(5);
    const qs = new URLSearchParams({ date_from: t, date_to: t5, limit: '500', tz: 'America/Sao_Paulo' });
    if(league_id) qs.set('league', league_id);

    const raw = await fetch(`https://sports.bzzoiro.com/api/events/?${qs}`, {
      headers: { Authorization: `Token ${BSD_TOKEN}` }
    }).then(r => r.json());

    let todos = (raw.results || []).map(normEvento);
    let jogos = todos.filter(ev => {
      const s = (ev.status || '').toLowerCase();
      if(['finished','ft','full-time','ended','canceled','postponed','live','inprogress','1h','ht','2h'].some(x=>s.includes(x))) return false;
      if(!ev.home_team || !ev.away_team || ev.home_team==='TBD' || ev.away_team==='TBD') return false;
      return true;
    });

    console.log(`[value-bets] v1: ${todos.length} eventos, ${jogos.length} futuros com times definidos`);

    if(!jogos.length){
      return res.json({ results:[], total:0, source:'internal_calc',
        _diag: { eventos_total: todos.length, janela:`${t} a ${t5}`, futuros_com_time:0,
          note:'Jogos futuros sem times definidos ainda (ex: Copa do Mundo aguardando chave)' } });
    }

    // Enriquece odds usando a função já funcionando nas rotas hoje/amanhã
    jogos = await enriquecerOdds(jogos);
    const comOdds = jogos.filter(ev => ev.odds_home && ev.odds_away);
    console.log(`[value-bets] com odds: ${comOdds.length}/${jogos.length}`);

    if(!comOdds.length){
      return res.json({ results:[], total:0, source:'internal_calc',
        _diag: { eventos_total: todos.length, futuros_com_time: jogos.length, futuros_com_odds:0,
          note:'Nenhum jogo futuro tem odds disponíveis ainda' } });
    }

    // Calcula edge — predição via bsd() que usa BASE_URL=/api/v2 corretamente
    const valueBets = [];
    const CONC = 6;
    const chunks = [];
    for(let i=0;i<comOdds.length;i+=CONC) chunks.push(comOdds.slice(i,i+CONC));

    for(const chunk of chunks){
      const res2 = await Promise.allSettled(chunk.map(async ev => {
        try{
          const pred = await bsd(`/events/${ev.id}/prediction/`);
          const mRes = pred?.markets?.match_result || {};
          let probH = mRes.prob_home, probA = mRes.prob_away, probD = mRes.prob_draw;
          if(probH==null || probA==null) return null;
          const isDec = probH<=1 && probA<=1;
          if(isDec){ probH*=100; probA*=100; if(probD!=null) probD*=100; }
          const oh=parseFloat(ev.odds_home||0), ox=parseFloat(ev.odds_draw||0), oa=parseFloat(ev.odds_away||0);
          if(!oh||!oa) return null;
          const bets=[];
          const eH=((probH/100)*oh-1)*100, eD=probD!=null&&ox?((probD/100)*ox-1)*100:null, eA=((probA/100)*oa-1)*100;
          if(eH>=minEdgePct) bets.push({selection:'home',label:`${ev.home_team} vence`,odd:oh,prob:+probH.toFixed(1),edge:+eH.toFixed(1)});
          if(eD!=null&&eD>=minEdgePct) bets.push({selection:'draw',label:'Empate',odd:ox,prob:+probD.toFixed(1),edge:+eD.toFixed(1)});
          if(eA>=minEdgePct) bets.push({selection:'away',label:`${ev.away_team} vence`,odd:oa,prob:+probA.toFixed(1),edge:+eA.toFixed(1)});
          if(!bets.length) return null;
          return { evento:ev, bets };
        }catch(_){return null;}
      }));
      res2.forEach(r=>{ if(r.status==='fulfilled'&&r.value) valueBets.push(r.value); });
    }

    valueBets.sort((a,b)=>Math.max(...b.bets.map(x=>x.edge))-Math.max(...a.bets.map(x=>x.edge)));
    const sliced = valueBets.slice(0, parseInt(limit));
    console.log(`[value-bets] ${comOdds.length} avaliados → ${valueBets.length} com edge ≥${min_edge}%`);
    res.json({ results:sliced, total:sliced.length, source:'internal_calc', min_edge:minEdgePct });
