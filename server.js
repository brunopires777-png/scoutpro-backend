...
// ─────────────────────────────────────────────
// NOVAS ROTAS — Odds comparação, Best Odds, Polymarket, Venue
// ─────────────────────────────────────────────

// Compare odds — todas as casas para um jogo
app.get('/api/jogos/:id/compare-odds', async (req, res) => {
  try {
    const data = await bsd(`/events/${req.params.id}/odds/comparison/`)
      .catch(() => bsd(`/api/v2/events/${req.params.id}/odds/comparison/`));
    // Normalizar bookmakers
    const raw = data?.results || data?.bookmakers || [];
    const arr = Array.isArray(raw) ? raw : Object.entries(raw).map(([slug, mkts]) => {
      const x2 = (mkts?.['1x2'] || mkts || {});
      return { bookmaker: slug, home_win: x2.HOME || x2.home_win || null,
        draw: x2.DRAW || x2.draw || null, away_win: x2.AWAY || x2.away_win || null };
    });
    const bkms = arr.map(b => ({
      ...
      .sort((a,b) => (b.home_win||0) - (a.home_win||0));
    res.json({ bookmakers: bkms });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Best odds — melhor odd por mercado
app.get('/api/jogos/:id/best-odds', async (req, res) => {
  try {
    const data = await bsd(`/events/${req.params.id}/odds/`)
      .catch(() => null);
    const comp = await bsd(`/events/${req.params.id}/odds/comparison/`)
      .catch(() => null);
    const simple = data?.odds || {};
    const arr = comp?.results || comp?.bookmakers || [];
    // Calcular melhor odd por mercado
    let bestHome = { val: simple.home_win || 0, book: '' };
    let bestDraw = { val: simple.draw || 0, book: '' };
    let bestAway = { val: simple.away_win || 0, book: '' };
    (Array.isArray(arr) ? arr : []).forEach(b => {
      ...
      if (h > bestHome.val) { bestHome = { val: h, book: name }; }
      if (d > bestDraw.val) { bestDraw = { val: d, book: name }; }
      if (a > bestAway.val) { bestAway = { val: a, book: name }; }
    });
    res.json({
      home_win:  bestHome,
      draw:      bestDraw,
      away_win:  bestAway,
      over_25:   simple.over_25_goals || null,
      under_25:  simple.under_25_goals || null,
      btts_yes:  simple.btts_yes || null,
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Polymarket odds para um jogo
...
// START
// ─────────────────────────────────────────────
// ── WebSocket proxy — repassa eventos live da BSD para os clientes
let _bsdWs = null;
...
function conectarBSDWebSocket() {
  try {
    ...
    _bsdWs.on('close', () => {
      console.log('[WS] BSD desconectado, reconectando em 10s...');
      setTimeout(conectarBSDWebSocket, 10000);
    });
