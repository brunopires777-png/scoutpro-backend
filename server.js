  ...
  try {
    ...
    await Promise.all(sliced.map(async vb => {
      try {
        ...
        const aiText = typeof aiObj === 'string' ? aiObj : (aiObj?.text || null);
        vb.ai_preview = aiText || null;
        vb.score_ia = extrairPlacarDoTexto(aiText, vb.evento.home_team, vb.evento.away_team);
      } catch(_){ vb.ai_preview = null; vb.score_ia = null; }
    }));

    console.log(`[value-bets v2] ${preds.length} preds → ${valueBets.length} bets | ${sliced.filter(v=>v.score_ia).length} com placar IA`);
